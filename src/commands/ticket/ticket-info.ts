...
class TicketInfoCommand extends Command {
  ...
  async exec (msg: Message, args: ArgsI) {
    ...
    const getUser = async (id: string) => client.users.get(id) || await client.fetchUser(id).catch(x => null)
    ...
    const rankEmoji = typeof ticket.rating === 'number' ? ticket.rating > 5 ? Emojis.STRONKS : Emojis.NOT_STRONKS : Emojis.NOT_STRONKS
