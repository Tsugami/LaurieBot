...
export async function createMainMessage(server: Guild, channel: TextChannel, guildData: GuildController) {
  ...
  const embed = new Embed(server.client.user).setAuthor("Ticket's").setDescription('Descrição explicativa');
...
export async function findTicketTypeArg<T>(
  w: string,
  msg: Message,
  args: T,
  argsHandler: handlerT,
  checkIfAllClosed = true,
): Promise<TicketArgType> {
