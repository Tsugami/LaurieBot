...
export async function changeMainChannelHandler(msg: Message, newChannel: TextChannel) {
  ...
  if (oldChannel && ticket.messageId && oldChannel instanceof TextChannel) {
    const sent = await oldChannel.fetchMessage(ticket.messageId).catch(() => null);
    if (sent) sent.delete();
  }

  await setupMainChannel(msg.guild, newChannel, guildData);
  return msg.reply(
    `canal de ticket's alterado. ${oldChannel ? oldChannel.toString() : oldId} **>>>** ${newChannel.toString()}`,
  );
}
...
export async function changeCategoryHandler(msg: Message, category: CategoryChannel) {
  ...
  if (oldId) {
    ...
    text = `categoria de ticket's alterada. ${
      oldCategory ? oldCategory.name.toUpperCase() : oldId
...
export async function changeRoleChange(msg: Message, role: Role) {
  ...
  let text: string;
  if (oldId) {
    text = `cargo de ticket's alterada. ${
      oldRole ? oldRole.name.toUpperCase() : oldId
